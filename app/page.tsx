...
export default function Home() {
  ...
  async function collectBatch(provider?: 'KIS') {
    ...
    try {
      const data = await api<{ batch: { calls: number; successful: number }; kis: { successful: number; requested: number } | null; quota: ProviderQuota }>(
        '/api/admin/collect', { method: 'POST', body: JSON.stringify({ maxCalls: 10, provider }) }, adminToken,
      );
      await Promise.all([loadAssets(), loadDashboard()]);
      const label = provider === 'KIS' ? 'KIS 연결 자산 수집' : '전체 자산 순환 수집';
    ...
    <main className="min-h-screen lg:grid lg:grid-cols-[248px_1fr]">
      ...
      <section className="min-w-0">
        <header className="flex min-h-20 items-center justify-between border-b bg-white/70 px-5 py-3 backdrop-blur-xl sm:px-8">
          <div>
            <p className="text-xs font-semibold uppercase tracking-[0.18em] text-muted-foreground">Market Intel</p>
        ...
        <div className="mx-auto min-w-0 max-w-[1500px] p-4 sm:p-8">
  ...
  return <div className="min-w-0 space-y-5 sm:space-y-6">
    ...
    <section className="grid gap-4 xl:grid-cols-[1fr_1.5fr]">
      <article className={`rounded-2xl border p-5 shadow-[0_10px_30px_rgb(30_58_95/5%)] ${stanceTone}`}>
        <div className="flex flex-wrap items-start justify-between gap-3"><div className="min-w-0"><p className="text-xs font-semibold uppercase tracking-[0.16em]">오늘의 판단</p><h2 className="mt-2 text-xl font-semibold">{kis.decision.title}</h2></div><span className="shrink-0 rounded-full bg-white/70 px-2.5 py-1 text-xs font-semibold">{kis.asOf ?? '수집 대기'}</span></div>
      ...
      <article className="min-w-0 rounded-2xl border bg-card p-5 shadow-[0_10px_30px_rgb(30_58_95/5%)]"><div className="flex flex-wrap items-start justify-between gap-3"><div className="min-w-0"><h2 className="font-semibold">한국 시장 수급 · 시장폭</h2><p className="mt-1 text-sm text-muted-foreground">외국인·기관 방향과 상승 종목 확산을 함께 봅니다.</p></div><span className="shrink-0 text-xs text-muted-foreground">KIS 조회 전용</span></div>
  ...
  return <div className="space-y-6">
    <section>
      ...
      <div className="grid gap-4 sm:grid-cols-2 xl:grid-cols-3">{rows.map(({ asset, snapshot: item }) => <button key={asset.id} type="button" onClick={() => setSelectedId(asset.id)} className={`rounded-2xl border bg-card p-5 text-left shadow-[0_10px_30px_rgb(30_58_95/5%)] transition hover:-translate-y-0.5 hover:border-blue-300 ${effectiveId === asset.id ? 'border-blue-500 ring-2 ring-blue-100' : ''}`}>
        ...
        <div className="mt-4 grid grid-cols-2 gap-2 text-xs sm:grid-cols-3 [&>span:last-child]:col-span-2 sm:[&>span:last-child]:col-span-1"><span className="rounded-lg bg-slate-50 p-2">Score <strong className="block text-sm">{number(item?.compositeScore, 0)}</strong></span><span className="rounded-lg bg-slate-50 p-2">MA20 <strong className="block text-sm">{number(item?.ma20)}</strong></span><span className="rounded-lg bg-slate-50 p-2">RSI <strong className="block text-sm">{number(item?.rsi14, 1)}</strong></span></div>
      </button>)}</div>
    </section>
    {selected && <section className="overflow-hidden rounded-2xl border bg-card shadow-[0_10px_30px_rgb(30_58_95/5%)]">
      ...
      <div className="grid gap-6 p-5 sm:p-6 xl:grid-cols-[1.3fr_1fr]">
        ...
        <div className="grid grid-cols-1 gap-3 min-[360px]:grid-cols-2 content-start">{[
...
function Reports({ reports, detail, events, loading, onSelect }: { reports: ReportListItem[]; detail: ReportDetail | null; events: EconomicEvent[]; loading: boolean; onSelect: (id: number) => void }) {
  ...
  return <div className="grid min-w-0 gap-5 sm:gap-6 xl:grid-cols-[330px_minmax(0,1fr)]">
    <aside className="min-w-0 overflow-hidden rounded-2xl border bg-card shadow-[0_10px_30px_rgb(30_58_95/5%)]">
    ...
    {detail && <article className="min-w-0 rounded-2xl border bg-card p-5 shadow-[0_10px_30px_rgb(30_58_95/5%)] sm:p-7">
      ...
      <section className="mt-4 rounded-xl bg-slate-50 p-4"><h3 className="font-semibold">다가오는 주요 이벤트</h3><div className="mt-3 space-y-2">{events.filter((event) => event.status === 'SCHEDULED' && new Date(event.scheduledAt) >= new Date()).slice(0, 3).map((event) => <div key={event.id} className="flex flex-wrap items-center justify-between gap-3 text-sm"><span className="min-w-0"><strong>{event.eventName}</strong><span className="ml-2 text-muted-foreground">{new Date(event.scheduledAt).toLocaleString('ko-KR')}</span></span><span className="shrink-0 font-semibold text-blue-700">Impact {event.expectedImpact}</span></div>)}{!events.some((event) => event.status === 'SCHEDULED' && new Date(event.scheduledAt) >= new Date()) && <p className="text-sm text-muted-foreground">등록된 예정 이벤트가 없습니다.</p>}</div></section>
    </article>}
  </div>;
}
